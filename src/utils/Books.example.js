/**
 * Follow the formatting here for each Book you wish to add. Spelling for the keys must be a perfect match, so please
 * double check your spelling and feel free to copy-paste the final, empty Book object to save time.
 *
 * Beware Book titles or descriptions that may contain either the " or ' characters; if you encounter this,
 * you may want to switch to using the ` character or escape the problematic character like so: \' or \".
 *
 */

const Books = [
  {
    title: 'Spider-Man: No Way Home',
    description:
      "With Spider-Man's identity now revealed, Peter asks Doctor Strange for help. When a spell goes wrong, dangerous foes from other worlds start to appear, forcing Peter to discover what it truly means to be Spider-Man.",
    imageURL:
      'https://m.media-amazon.com/images/M/MV5BZWMyYzFjYTYtNTRjYi00OGExLWE2YzgtOGRmYjAxZTU3NzBiXkEyXkFqcGdeQXVyMzQ0MzA0NTM@._V1_FMjpg_UX1000_.jpg',
    releaseYear: '2021',
    bookRating: 'PG-13',
    runTime: '2h 28m',
    authors: ['Jon Watts'],
    writers: ['Chris McKenna', 'Erik Sommers', 'Stan Lee'],
    actors: ['Tom Holland', 'Zendaya', 'Benedict Cumberbatch'],
  },
  {
    title: 'The Matrix',
    description:
      'When a beautiful stranger leads computer hacker Neo to a forbidding underworld, he discovers the shocking truth--the life he knows is the elaborate deception of an evil cyber-intelligence.',
    imageURL:
      'https://m.media-amazon.com/images/M/MV5BNzQzOTk3OTAtNDQ0Zi00ZTVkLWI0MTEtMDllZjNkYzNjNTc4L2ltYWdlXkEyXkFqcGdeQXVyNjU0OTQ0OTY@._V1_.jpg',
    releaseYear: '1999',
    bookRating: 'R',
    runTime: '2h 16m',
    authors: ['Lana Wachowski', 'Lilly Wachowski'],
    writers: ['Lana Wachowski', 'Lilly Wachowski'],
    actors: ['Keeanu Reeves', 'Laurence Fishburne', 'Carrie-Anne Moss'],
  },
  {
    title: 'The Godfather',
    description:
      'The Godfather follows Vito Corleone, Don of the Corleone family, as he passes the mantel to his unwilling son, Michael.',
    imageURL:
      'https://m.media-amazon.com/images/M/MV5BM2MyNjYxNmUtYTAwNi00MTYxLWJmNWYtYzZlODY3ZTk3OTFlXkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg',
    releaseYear: '1972',
    bookRating: 'R',
    runTime: '2h 55m',
    authors: ['Francis Ford Coppola'],
    writers: ['Francis Ford Coppola', 'Mario Puzo'],
    actors: ['Marlon Brando', 'Al Pacino', 'James Caan'],
  },
  {
    title: 'The Godfather: Part II',
    description:
      'The early life and career of Vito Corleone in 1920s New York City is portrayed, while his son, Michael, expands and tightens his grip on the family crime syndicate.',
    imageURL:
      'https://m.media-amazon.com/images/M/MV5BMWMwMGQzZTItY2JlNC00OWZiLWIyMDctNDk2ZDQ2YjRjMWQ0XkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg',
    releaseYear: '1974',
    bookRating: 'R',
    runTime: '3h 22m',
    authors: ['Francis Ford Coppola'],
    writers: ['Francis Ford Coppola', 'Mario Puzo'],
    actors: ['Al Pacino', 'Robert De Niro', 'Robert Duvall'],
  },
  {
    title: 'The Shawshank Redemption',
    description:
      'Two imprisoned men bond over a number of years, finding solace and eventual redemption through acts of common decency.',
    imageURL:
      'https://fesapusewebsite.blob.core.windows.net/fathom/1401408f3721442f27b6cbb56ab95599.jpg',
    releaseYear: '1994',
    bookRating: 'R',
    runTime: '2h 22m',
    authors: ['Frank Darabont'],
    writers: ['Stephen King', 'Frank Darabont'],
    actors: ['Tim Robbins', 'Morgan Freeman', 'Bob Gunton'],
  },
  {
    title: 'In the Heights',
    description:
      'A film version of the Broadway musical in which Usnavi, a sympathetic New York bodega owner, saves every penny every day as he imagines and sings about a better life.',
    imageURL:
      'https://m.media-amazon.com/images/M/MV5BMzM1ZDY0YTktZTYzZi00MjZjLTllMDMtMmNlMmM5NmY4ZjllXkEyXkFqcGdeQXVyMTA1OTcyNDQ4._V1_.jpg',
    releaseYear: '2021',
    bookRating: 'PG-13',
    runTime: '2h 23m',
    authors: ['Jon M. Chu'],
    writers: ['Lin-Manuel Miranda', 'Quiara Alegría Hudes'],
    actors: ['Anthony Ramos', 'Corey Hawkins', 'Leslie Grace'],
  },
  {
    title: '',
    description: '',
    imageURL: '',
    releaseYear: '',
    bookRating: '',
    runTime: '',
    authors: [''],
    writers: [''],
    actors: ['', ''],
  },
];

export default Books;
